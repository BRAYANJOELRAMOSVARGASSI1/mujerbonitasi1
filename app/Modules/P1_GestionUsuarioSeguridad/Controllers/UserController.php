...
namespace App\Modules\P1_GestionUsuarioSeguridad\Controllers;
...
class UserController extends Controller
{
    ...
    /**
     * [CU01] - Gestión de Usuarios: Listado
     */
    ...
    /**
     * [CU01] - Gestión de Usuarios: Creación (Acción)
     */
    ...
    /**
     * [CU06] - Gestión de Seguridad: Desbloqueo de Usuarios
     */
    public function unblock(Request $request, User $user)
    {
        ...
        ActivityLog::create([
            ...
            'description' => 'El administrador desbloqueó al usuario: ' . $user->name . ' (' . $user->email . ')',
            'ip_address' => $request->ip() ?? 'No disponible',
            'browser' => $request->header('user-agent') ?? 'No disponible',
        ]);

        return redirect()->route('users.index')
            ->with('status', 'Usuario desbloqueado correctamente. El contador de fallos ha sido restablecido.');
    }

    /**
     * [CU02/CU03] - Asignación de Roles y Permisos a Usuarios
     */
    ...
    /**
     * [CU05] - Bitácora de Auditoría
     */
    public function bitacora(Request $request): View
    {
        if (! Auth::user()->hasAnyRole(['admin', 'super-admin']) && ! Auth::user()->can('ver bitacora')) {
            abort(403, 'Acceso Denegado. Esta sección es exclusiva para la dueña del negocio y soporte técnico.');
        }
