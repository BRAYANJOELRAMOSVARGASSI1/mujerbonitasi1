        ...
        <form method="POST" action="{{ route('herramientas.update', $herramienta) }}">
            ...
            <div class="card-body" style="background-color:#f8f9fa; padding:2rem;">
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3"><label class="form-label" style="color:#495057; font-weight:500;">Estado</label><select name="estado" class="form-select" style="border:1px solid #dee2e6; border-radius:8px; padding:0.75rem;">@foreach($estados as $key => $label)<option value="{{ $key }}" {{ old('estado', $herramienta->estado) == $key ? 'selected' : '' }}>{{ $label }}</option>@endforeach</select></div>
                    <div class="col-md-4 mb-3"><label class="form-label" style="color:#495057; font-weight:500;">Fecha Adquisición</label><input type="date" name="fecha_adquisicion" class="form-control" value="{{ old('fecha_adquisicion', $herramienta->fecha_adquisicion instanceof \Carbon\Carbon ? $herramienta->fecha_adquisicion->format('Y-m-d') : (is_string($herramienta->fecha_adquisicion) ? date('Y-m-d', strtotime($herramienta->fecha_adquisicion)) : '')) }}" style="border:1px solid #dee2e6; border-radius:8px; padding:0.75rem;"></div>
